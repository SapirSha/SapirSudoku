...
namespace SapirSudoku.src.DataStructures
{
    public class BitSet : IEnumerable<int>
    {
        ...
        public static BitSet Union(params BitSet[] sets)
        {
            ...
            {
                int looplen = MathUtilities.Min(s.set.Length, unionSet.set.Length);
        ...
        public static BitSet Intersection(params BitSet[] sets)
        {
            ...
            {
                int looplen = MathUtilities.Min(sets[setIndex].set.Length, interSet.set.Length);
                int i;
                for (i = 0; i < looplen; i++)
        ...
        public static BitSet Difference(params BitSet[] sets)
        {
            ...
            foreach (BitSet s in sets)
            {
                int looplen = MathUtilities.Min(s.set.Length, diffSet.set.Length);
                int i;
                for (i = 0; i < looplen; i++)
        ...
        public static BitSet Subtract(BitSet subtracted, BitSet subtracter)
        {
            ...
            int looplen = MathUtilities.Min(subtracted.set.Length, subtracter.set.Length);
            for (int i = 0; i < looplen; i++)
        ...
        {
            return Union(this, superset).Equals(superset);
        }

        public bool IsSuperSetOf(BitSet superset)
        {
            return Union(superset, this).Equals(this);
        }
